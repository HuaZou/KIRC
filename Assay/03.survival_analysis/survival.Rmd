---
title: "survival analysis: the autoencoder features related to survial status"
...
feature <- fread("../../Result/profile/Autoencoder_top100_newfeatures.csv")
```
...
get_coxPh <- function(profile=feature, 
  ...
  res <- apply(datprf, 2, function(x, phe){ 
    ...
    arrange(FDR) %>%
    mutate(Status=ifelse(FDR < 0.01, "Remain", "Discard"))
...
write.csv(cox_ph, "../../Result/feature/survival_AEfeatures.csv", row.names = F)
```
...
write.csv(feature_trim, "../../Result/feature/Autoencoder_Remain_features.csv", row.names = F)
```
